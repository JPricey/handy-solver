    ...
    match allegiance {
        ...
        Allegiance::Hero => {
            ...
            for (row_idx, row) in active_face.rows.iter().enumerate() {
                let base_row_state = state
                    .clone()
                    .append_event(Event::PickRow(row_idx, *active_card));
                let row_outcomes = resolve_player_row(&base_row_state, row, active_idx);
                all_outcomes.extend(row_outcomes);
            }

            all_outcomes.push(state.clone().append_event(Event::SkipTurn));
            all_outcomes
        }
    }
}
        ...
        Some(Condition::Energy(amount)) => {
            ...
            for energy_combo in energy_options.iter().copied().combinations(amount as usize) {
                ...
                for used_energy_idx in &energy_combo {
                    ...
                    let used_energy_card = energy_used_state.get_pile()[*used_energy_idx];
                    energy_used_state
                        .mut_append_event(Event::PayEnergy(*used_energy_idx, used_energy_card));
                }
    ...
    match wrapped_action.action {
        ...
        Action::Teleport => {
            ...
            for first_idx in active_idx + 1..pile.len() {
                ...
                for second_idx in first_idx + 1..pile.len() {
                    ...
                    let mut new_state = state.clone().append_event(Event::Teleport(
                        first_idx,
                        pile[first_idx],
                        second_idx,
                        pile[second_idx],
        ...
        Action::Hit(range) => {
            ...
            for target_idx in active_idx + 1..range_cap {
                ...
                let blockers_results =
                    find_blockers_for_hit_outcomes(&state, active_idx, range_cap, target_idx);

                if blockers_results.len() > 0 {
            ...
            for i in 0..arrow_targets.len() {
...

                {
                    for first_arrow_state in
                        attack_card_get_all_outcomes(&state, target_idx_1, HitType::Arrow)
                    {
                        for j in i + 1..arrow_targets.len() {
                        ...
                        // Only use 1 arrow
                        results.push(first_arrow_state);
                    }
                }
            }
        }
            ...
            for target_idx in active_idx + 1..pile.len() {
                ...
                if target_card_ptr.get_card_def().faces[rotated_key].health == target_health {
                    let mut new_state = state
                        .clone()
                        .append_event(Event::Manouver(target_idx, target_card_ptr));
...
fn move_card_by_up_to_amount<T: EngineGameState>(
    ...
    direction: i32,
    event_constructor: &dyn Fn(usize, CardPtr, usize) -> Event,
) -> Vec<T> {
...
fn _inner_move_card_by_up_to_amount<T: EngineGameState>(
    state: &T,
    target_idx: usize,
    distance_remaining: i32,
    distance_so_far: usize,
            ...
            .intersects(Features::Trap)
    {
        ...
        let hit_options = attack_card_get_all_outcomes(&new_state, swap_with_idx, HitType::Trap);

        for hit_option in hit_options {
            results.push((hit_option, distance_so_far + 1));
        }
    }

    if results.len() == 0 {
        results.push((new_state, distance_so_far + 1));
    }

    if distance_remaining > 1 {
        let mut new_results = vec![];
        for current_result in &results {
            let new_children = _inner_move_card_by_up_to_amount(
                &current_result.0,
                swap_with_idx,
            ...
            new_results.extend(new_children);
        }
...
fn resolve_enemy_turn_no_swarm<T: EngineGameState>(
    ...
    for (row_idx, row) in active_face.rows.iter().enumerate() {
        let state_with_row_idx = state
            .clone()
            .append_event(Event::PickRow(row_idx, state.get_pile()[active_idx]));
        let row_outcomes =
    ...
    return vec![state.clone().append_event(Event::SkipTurn)];
}
...
fn get_reaction_assist_results<T: EngineGameState>(
    ...
    final_results.push(react_cost_state);
...
fn bottom_top_card<T: EngineGameState>(state: &mut T) {
    state.mut_append_event(Event::BottomCard(state.get_pile()[0]));
...
pub fn is_game_winner(pile: &Pile) -> Option<Allegiance> {
    ...
    for card in pile.iter() {
        if card.get_active_face().health != Health::Empty {
            match card.get_active_face().allegiance {
                Allegiance::Hero => enemy_wins = false,
                Allegiance::Baddie => player_wins = false,
                Allegiance::Werewolf => (),
            }
        }
    }
...
fn perform_card_self_action(self_action: SelfAction, active_card: &mut CardPtr) {
    match self_action {
        SelfAction::Rotate => {
...
mod tests {
    ...
    fn test_bug7() {
        ...
        assert_eq!(
            new_states[0].events,
            vec![Event::SkipTurn, Event::BottomCard(string_to_card_ptr("9A"))]
        )
    }
    ...
    fn test_quicken_delay() {
        // For no reason in particular. Just to get some test data before a refactor
